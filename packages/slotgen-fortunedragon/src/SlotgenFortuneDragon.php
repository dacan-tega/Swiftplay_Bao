...
class SlotgenFortuneDragon
{
    public static function getGame()
    {
        // $api_url = route('web.fortunedragon.site.launch');
        ...
        if (File::exists($gameFile)) {
            ...
            $logo_url = url('uploads/games/' . $gameFolder . '/fortunedragon-logo.png');
        } else {
            $gameFolder = '';
    ...
    {
        $setting = (object) SlotgenFortuneDragonConfig::first();
        ...
        $dataType = 'normal';
        SlotgenFortuneDragon::spinConfig($path, $dataType);
        ...
        if ($USE_SEAMLESS) {
            $user = SlotgenFortuneDragon::PlayerEntityId($userName);
        } else {
            $user = SlotgenFortuneDragon::PlayerEntityId($playerId);
        }
        ...
        if (! $user) {
            if ($gameName) {
                ...
                $playerData = new FortuneDragonPlayer;
                $playerData->fill($data);
                $playerData->save();
                $user = FortuneDragonPlayer::where('uuid', $playerData->uuid)->first();
                // dd($user);
                if (! $playerData) {
