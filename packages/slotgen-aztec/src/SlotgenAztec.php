...
class SlotgenAztec
{
    ...
    public static function LaunchGame($launchData)
    {
        ...
        $dataType = 'normal';
        SlotgenAztec::spinConfig($path, $dataType);
        ...
        if ($USE_SEAMLESS) {
            $user = SlotgenAztec::PlayerEntityId($userName);
        } else {
            $user = SlotgenAztec::PlayerEntityId($playerId);
        }
        ...
        if (!$user) {
            if ($gameName) {
                ...
                $betSize = (float) $currency->betsize[0];
                ...
                $playerData = new AztecPlayer();
                $playerData->fill($data);
                $playerData->save();
                $user = AztecPlayer::where('uuid', $playerData->uuid)->first();
                // dd($user);
                if (!$playerData) {
