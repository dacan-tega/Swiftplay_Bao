...
namespace Slotgen\SlotgenCaptainsBounty\Filament\Pages;
...
use Slotgen\SlotgenCaptainsBounty\Models\SlotgenGame;
...
class SlotgenCaptainsBountyConfigPage extends Page
{
    use InteractsWithForms;

    protected static ?string $navigationIcon = 'heroicon-o-document-text';

    protected static string $view = 'slotgen-captainsbounty::filament.pages.config-page';

    public ?array $data = [];

    public SlotgenCaptainsBountyConfig $setting;

    public function mount(): void
    {
        $rtpConfig = SlotgenCaptainsBountyConfig::first();
        if (! empty($rtpConfig)) {
    ...
    public function submit(): void
    {
        ...
        try {
            // dd($this->data);
            $setting = SlotgenCaptainsBountyConfig::first();
            if (! empty($setting)) {
                ...
                // dd($this->data);
                if (SlotgenCaptainsBountyConfig::create($this->data)) {
                    Notification::make()
                        ->title('Setting add')
    ...
    public function extractFile($fileName, $language = 'en')
    {
        ...
        if ($folder != null) {
            ...
            // $data = [
            //     'game_name'        => $name,
            //     'game_class'     => "Slotgen\\SlotgenCaptainsBounty\\SlotgenCaptainsBounty",
            // ];
            // $slotgen->fill($data);
            // $slotgen->save();
            $gamePrivateFolder = storage_path('app/private/captains_bounty');
            $localtionStorage = 'app/private/captains_bounty';
            ...
            //File::copy(__DIR__ . '/../../../resources/games/captainsbounty-logo.png', $gamePublicFolder . '/captainsbounty-logo.png');
        }
    }
}
