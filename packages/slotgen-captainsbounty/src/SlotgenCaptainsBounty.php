...
class SlotgenCaptainsBounty
{
    public static function getGame()
    {
        // $api_url = route('web.captainsbounty.site.launch');
        ...
        if (File::exists($gameFile)) {
            ...
            $logo_url = url('uploads/games/' . $gameFolder . '/captainsbounty-logo.png');
        } else {
            $gameFolder = '';
    ...
    public static function LaunchGame($launchData)
    {
        ...
        $dataType = 'normal';
        SlotgenCaptainsBounty::spinConfig($path, $dataType);
        ...
        if ($USE_SEAMLESS) {
            $user = SlotgenCaptainsBounty::PlayerEntityId($userName);
        } else {
            $user = SlotgenCaptainsBounty::PlayerEntityId($playerId);
        }
        ...
        if (! $user) {
            if ($gameName) {
                ...
                $playerData = new CaptainsBountyPlayer;
                $playerData->fill($data);
                $playerData->save();
                $user = CaptainsBountyPlayer::where('uuid', $playerData->uuid)->first();
                // dd($user);
                if (! $playerData) {
